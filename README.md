...
한 가지 명심해야 할 점은 단락 평가가 존재한다는 것이다.  
AND 조건일 경우, 첫 번째 조건이 거짓이면 두 번째 조건은 확인하지 않는다. 첫 번재 조건에서 이미 거짓이면 두 번재 조건에 상관없이 거짓이기 때문이다.  
OR 조건일 경우, 첫 번째 조건이 참이면 역시 두 번째 조건은 확인하지 않는다. 첫 번째 조건에서 이미 참이면 두 번째 조건에 상관없이 참이기 때문이다.  
...
해당 예제의 콘솔 결과는 따로 첨부하지 않는다.  
random 값에 의해 실행할 때마다 값이 바뀌어 매번 다른 결과가 노출되기 때문에 직접 실행하여 확인하는게 좋다.
